-- Script de récupération complète pour restaurer toutes les tables
...
CREATE TRIGGER IF NOT EXISTS update_article_comments_updated_at
...
CREATE POLICY IF NOT EXISTS "Projects are publicly readable" ON public.projects FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Authenticated users can manage projects" ON public.projects FOR ALL USING (auth.role() = 'authenticated');

-- Blog posts : lecture publique des publiés, modification authentifiée
CREATE POLICY IF NOT EXISTS "Published blog posts are publicly readable" ON public.blog_posts FOR SELECT USING (published = true OR auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "Authenticated users can manage blog posts" ON public.blog_posts FOR ALL USING (auth.role() = 'authenticated');

-- Catégories : lecture publique, modification authentifiée
CREATE POLICY IF NOT EXISTS "Categories are publicly readable" ON public.categories FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Authenticated users can manage categories" ON public.categories FOR ALL USING (auth.role() = 'authenticated');

-- Interactions d'articles : insertion publique, lecture/modification authentifiée
CREATE POLICY IF NOT EXISTS "Anyone can view articles" ON public.article_views FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Authenticated users can read views" ON public.article_views FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Anyone can like articles" ON public.article_likes FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Authenticated users can read likes" ON public.article_likes FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Anyone can comment on articles" ON public.article_comments FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Approved comments are publicly readable" ON public.article_comments FOR SELECT USING (approved = true OR auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "Authenticated users can manage comments" ON public.article_comments FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Anyone can share articles" ON public.article_shares FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Authenticated users can read shares" ON public.article_shares FOR SELECT USING (auth.role() = 'authenticated');

-- About sections : lecture publique des actives, modification authentifiée
CREATE POLICY IF NOT EXISTS "Active about sections are publicly readable" ON public.about_sections FOR SELECT USING (is_active = true OR auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "Authenticated users can manage about sections" ON public.about_sections FOR ALL USING (auth.role() = 'authenticated');

-- Messages de contact : insertion publique, lecture/modification authentifiée
CREATE POLICY IF NOT EXISTS "Anyone can send contact messages" ON public.contact_messages FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Authenticated users can read contact messages" ON public.contact_messages FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "Authenticated users can update contact messages" ON public.contact_messages FOR UPDATE USING (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "Authenticated users can delete contact messages" ON public.contact_messages FOR DELETE USING (auth.role() = 'authenticated');
